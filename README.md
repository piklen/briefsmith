...
Briefsmith is a **Prompt Quality Gate / Task Brief Compiler** for AI coding hosts such as Codex, Claude Code, and OpenCode.

It helps turn vague user requests like "optimize this" into an executable task brief by combining:
...
- `[.agents/skills/prompt-memory/SKILL.md](.agents/skills/prompt-memory/SKILL.md)` 是仓库内唯一权威 skill 源。
- `templates/*` 是从 canonical skill 派生出来的宿主快照，不再作为手写真源维护。
- `briefsmith adapters install <host>` 和 `briefsmith adapters install all` 负责自动落盘宿主侧配置；如缺少运行时产物，会先自动执行 `npm run build`。
...
Try preflight on a vague request:
...
- `skip`：当前项目已通过 `briefsmith stop` 关闭 prompt 检查。
...
- 当你执行 `briefsmith policy threshold codex 0.62` 时，Codex 的默认槽位阈值会一起刷新到 `0.62`。
- 当你再执行 `briefsmith policy threshold codex success_criteria 0.58` 时，只覆盖 `success_criteria`，其他槽位保持默认值。
- `briefsmith start` / `briefsmith stop` 只切换 `enabled` 和 `mode`，不会清掉你已经设置的 host / slot 阈值。
...
```bash
briefsmith compile "优化这个导入逻辑，不要改变外部命令行为，输出一个 superpowers 可直接消费的任务说明" --framework superpowers
```
...
```bash
briefsmith preflight "优化一下这个导入逻辑" --host codex --json
briefsmith preflight "优化一下这个导入逻辑" --host opencode --framework superpowers --json
```
...
briefsmith policy mode off
briefsmith policy mode suggest
briefsmith policy mode auto-compile
...
- 协作规范：见 [`CODE_OF_CONDUCT.md`](CODE_OF_CONDUCT.md)
...
4. Release workflow serializes publishes so only one `main` release runs at a time
...
4. 仓库公开后切到 npm trusted publishing / provenance。
